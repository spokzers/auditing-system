            <!-- //////////////////////////////////////////////////////////////////////////// -->

            <!-- START CONTENT -->
            <section id="content">

                <!--start container-->
                <div class="container">



                    <!-- //////////////////////////////////////////////////////////////////////////// -->

                    <!--card stats start-->
                    <div id="card-stats">
                        <div class="row">
                            <div class="col s12 m6 l3">
                                <div class="card">
                                    <div class="card-content  green white-text">
                                        <p class="card-stats-title"><i class="mdi-social-person" style="font-size: 75px;"></i><br>Inspectors</p>
                                        <h5></h5>
                                        <h4 class="card-stats-number">566</h4>
                                    </div>
                                    <div class="card-action green darken-2">
                                    </div>
                                </div>
                            </div>
                            <div class="col s12 m6 l3">
                              <div class="card">
                                  <div class="card-content  purple white-text">
                                      <p class="card-stats-title"><i class="mdi-toggle-check-box" style="font-size: 75px;"></i><br>Audits Performed</p>
                                      <h5></h5>
                                      <h4 class="card-stats-number">566</h4>
                                  </div>
                                  <div class="card-action purple darken-2">
                                  </div>
                              </div>
                            </div>
                            <div class="col s12 m6 l3">
                              <div class="card">
                                  <div class="card-content  blue-grey white-text">
                                      <p class="card-stats-title"><i class="mdi-toggle-check-box" style="font-size: 75px;"></i><br>Scheduled Audits</p>
                                      <h5></h5>
                                      <h4 class="card-stats-number">566</h4>
                                  </div>
                                  <div class="card-action blue-grey darken-2">
                                  </div>
                              </div>
                            </div>
                            <div class="col s12 m6 l3">
                              <div class="card">
                                  <div class="card-content  deep-purple white-text">
                                    <p class="card-stats-title"><i class="mdi-toggle-check-box" style="font-size: 75px;"></i><br>Pending Audits</p>

                                      <h5></h5>
                                      <h4 class="card-stats-number">566</h4>
                                  </div>
                                  <div class="card-action deep-purple darken-2">
                                  </div>
                              </div>
                            </div>
                        </div>

                        <!-- Card Stats Second Row -->
                        <div class="row">
                            <div class="col s12 m6 l3">
                                <div class="card">
                                    <div class="card-content  red white-text">
                                        <p class="card-stats-title"><i class="mdi-social-group-add" style="font-size: 75px;"></i><br>Workers Waiting</p>
                                        <h5></h5>
                                        <h4 class="card-stats-number">566</h4>
                                    </div>
                                    <div class="card-action red darken-2">
                                    </div>
                                </div>
                            </div>
                            <div class="col s12 m6 l3">
                              <div class="card">
                                  <div class="card-content  orange white-text">
                                      <p class="card-stats-title"><i class="mdi-social-group-add" style="font-size: 75px;"></i><br>Workers Trained</p>
                                      <h5></h5>
                                      <h4 class="card-stats-number">566</h4>
                                  </div>
                                  <div class="card-action orange darken-2">
                                  </div>
                              </div>
                            </div>
                            <div class="col s12 m6 l3">
                              <div class="card">
                                  <div class="card-content  blue white-text">
                                      <p class="card-stats-title"><i class="mdi-social-group-add" style="font-size: 75px;"></i><br>Total Workers</p>

                                      <h5></h5>
                                      <h4 class="card-stats-number">566</h4>
                                  </div>
                                  <div class="card-action blue darken-2">
                                  </div>
                              </div>
                            </div>
                            <div class="col s12 m6 l3">
                              <div class="card">
                                  <div class="card-content  pink   white-text">
                                    <p class="card-stats-title"><i class="mdi-action-assignment" style="font-size: 75px;"></i><br>Facilities</p>
                                      <h5></h5>
                                      <h4 class="card-stats-number">566</h4>
                                  </div>
                                  <div class="card-action pink darken-2">
                                  </div>
                              </div>
                            </div>
                        </div>

                    </div>
                    <!--card stats end-->

                    <!-- //////////////////////////////////////////////////////////////////////////// -->

                    <!--card widgets start-->
                    <div id="card-widgets">
                        <div class="row">
                          <h4 class="header" style="padding-left:20px;"> Violations Summary (Based on Inspection Results by Severity)</h4>
                            <div class="col s12 m12 l4">
                              <div class="card">
                                <div class="card-action blue">
                                  <div class="card-title">
                                    <h5>Overall</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="donut-example"></div>
                                   <div class="center">
                                     <ul>
                                       <li>High: 60</li>
                                       <li>Low: 60</li>
                                       <li>Medium: 60</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>

                            <div class="col s12 m12 l4">
                              <div class="card">
                                <div class="card-action amber">
                                  <div class="card-title">
                                    <h5>Environmental & Health Inspections</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="donut-example1"></div>
                                   <div class="center">
                                     <ul>
                                       <li>High: 60</li>
                                       <li>Low: 60</li>
                                       <li>Medium: 60</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>

                            <div class="col s12 m12 l4">
                              <div class="card">
                                <div class="card-action red">
                                  <div class="card-title">
                                    <h5>Market Control Inspections</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="donut-example2"></div>
                                   <div class="center">
                                     <ul>
                                       <li>High: 60</li>
                                       <li>Low: 60</li>
                                       <li>Medium: 60</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>

                        </div>

                        <div class="row">
                          <h4 class="header" style="padding-left:20px;"> Violations Summary (Based on Penalty Amounts)</h4>
                            <div class="col s12 m12 l4">
                              <div class="card">
                                <div class="card-action blue">
                                  <div class="card-title">
                                    <h5>Overall</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="donut-example3"></div>
                                   <div class="center">
                                     <ul>
                                       <li>High: 60</li>
                                       <li>Low: 60</li>
                                       <li>Medium: 60</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>

                            <div class="col s12 m12 l4">
                              <div class="card">
                                <div class="card-action amber">
                                  <div class="card-title">
                                    <h5>Environmental & Health Inspections</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="donut-example4"></div>
                                   <div class="center">
                                     <ul>
                                       <li>High: 60</li>
                                       <li>Low: 60</li>
                                       <li>Medium: 60</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>

                            <div class="col s12 m12 l4">
                              <div class="card">
                                <div class="card-action amber">
                                  <div class="card-title">
                                    <h5>License Status graph</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="license-status"></div>
                                   <div class="center">
                                     <ul>
                                       <li>Never Issued: 60</li>
                                       <li>Expired: 60</li>
                                       <li>Expiring this month: 60</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>



                        </div>

                        <div class="row">
                          <h4 class="header" style="padding-left:20px;"> Audit And Facilities Result</h4>
                            <div class="col s12 m12 l6">
                              <div class="card">
                                <div class="card-action blue">
                                  <div class="card-title">
                                    <h5>Facilities Result</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="facility-result"></div>
                                   <div class="center">
                                     <ul>
                                       <li>Pass: 60</li>
                                       <li>Fail: 60</li>
                                       <li>Need for Improvement: 60</li>
                                       <li>Warning: 45</li>
                                       <li>Not given: 10</li>
                                       <li>Office Hearing: 25</li>
                                       <li>Closure: 10</li>
                                       <li>Warning and office hearing: 10</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>

                            <div class="col s12 m12 l6">
                              <div class="card">
                                <div class="card-action amber">
                                  <div class="card-title">
                                    <h5>Audit Result</h5>
                                  </div>

                                </div>
                                  <div class="card-content-bg">
                                   <div id="audit-result"></div>
                                   <div class="center">
                                     <ul>
                                       <li>Audit on time: 60</li>
                                       <li>Audit Pending: 60</li>
                                       <li>Audit Delayed: 60</li>
                                     </ul>
                                   </div>
                                  </div>
                              </div>
                            </div>




                        </div>


                    </div>
                    <!--card widgets end-->

                    <!-- //////////////////////////////////////////////////////////////////////////// -->
                    <div>

                      <table class="striped">
                        <thead>
                          <tr>
                            <th data-field="id">Name</th>
                            <th data-field="name">Item Name</th>
                            <th data-field="price">Item Price</th>
                            <th data-field="actions">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr>
                            <td>Alvin</td>
                            <td>Eclair</td>
                            <td>$0.87</td>
                            <td><a href="#" class="btn-floating btn-large green waves-effect waves-light">View</a></td>
                          </tr>
                          <tr>
                            <td>Alan</td>
                            <td>Jellybean</td>
                            <td>$3.76</td>
                            <td><a href="#" class="btn-floating btn-large green waves-effect waves-light">View</a></td>
                          </tr>
                          <tr>
                            <td>Jonathan</td>
                            <td>Lollipop</td>
                            <td>$7.00</td>
                            <td><a href="#" class="btn-floating btn-large green waves-effect waves-light">View</a></td>
                          </tr>
                          <tr>
                            <td>Shannon</td>
                            <td>KitKat</td>
                            <td>$9.99</td>
                            <td><a href="#" class="btn-floating btn-large green waves-effect waves-light">View</a></td>
                          </tr>
                        </tbody>
                      </table>

                    </div>



                    <div class="your-audits">
                      <h4 class="header"> Your Audits</h4>
                      <table class="striped">
                        <thead>
                          <tr>
                            <th data-field="id">Audit ID</th>
                            <th data-field="name">Audit Location</th>
                            <th data-field="price">Date</th>
                            <th data-field="actions">Inspectors</th>
                            <th data-field="actions">Status</th>
                            <th data-field="actions">Disposal</th>
                            <th data-field="actions">Penalty</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr>
                            <td>Alvin</td>
                            <td>Eclair</td>
                            <td>$0.87</td>
                            <td>John Doe</td>
                            <td><span class="green white-text" style="padding:5px;">Verified</span></td>
                            <td>John Doe</td>
                            <td>John Doe</td>
                          </tr>
                          <tr>
                            <td>Alan</td>
                            <td>Jellybean</td>
                            <td>$3.76</td>
                            <td>John Doe</td>
                            <td><span class="green white-text" style="padding:5px;">Verified</span></td>
                            <td>John Doe</td>
                            <td>John Doe</td>

                          </tr>
                          <tr>
                            <td>Jonathan</td>
                            <td>Lollipop</td>
                            <td>$7.00</td>
                            <td>John Doe</td>
                            <td><span class="green white-text" style="padding:5px;">Verified</span></td>
                            <td>John Doe</td>
                            <td>John Doe</td>

                          </tr>
                          <tr>
                            <td>Shannon</td>
                            <td>KitKat</td>
                            <td>$9.99</td>
                            <td>John Doe</td>
                            <td><span class="green white-text" style="padding:5px;">Verified</span></td>
                            <td>John Doe</td>
                            <td>John Doe</td>

                          </tr>
                        </tbody>
                      </table>

                    </div>

                    <div class="all-audits">
                      <h4 class="header"> All Audits</h4>
                      <table class="striped">
                        <thead>
                          <tr>
                            <th data-field="id">Type</th>
                            <th data-field="name">Locattion</th>
                            <th data-field="price">Date</th>
                            <th data-field="actions">Inspectors</th>
                            <th data-field="actions">Status</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr>
                            <td>Alvin</td>
                            <td>Eclair</td>
                            <td>$0.87</td>
                            <td>John Doe</td>
                            <td><a href="#" class="btn blue">Report</a></td>

                          </tr>
                          <tr>
                            <td>Alan</td>
                            <td>Jellybean</td>
                            <td>$3.76</td>
                            <td>John Doe</td>
                            <td><a href="#" class="btn blue">Report</a></td>

                          </tr>
                          <tr>
                            <td>Jonathan</td>
                            <td>Lollipop</td>
                            <td>$7.00</td>
                            <td>John Doe</td>
                            <td><a href="#" class="btn blue">Report</a></td>
                          </tr>
                          <tr>
                            <td>Shannon</td>
                            <td>KitKat</td>
                            <td>$9.99</td>
                            <td>John Doe</td>
                            <td><a href="#" class="btn blue">Report</a></td>

                          </tr>
                        </tbody>
                      </table>

                    </div>

                </div>
                <!--end container-->
            </section>
            <!-- END CONTENT -->
